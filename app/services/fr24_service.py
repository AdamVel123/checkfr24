...
class FR24Service:
    ...
    def search(self, filters: FlightFilter, limit: int = 100) -> list[FlightView]:
        ...
        for raw in candidates[: max(limit * 5, 250)]:
            ...
            if self._match_filters(view, filters, skip_duration=False):
                result.append(view)
                if len(result) >= limit:
                    break
    ...
    def _to_view(self, raw: Any, details: dict[str, Any] | None = None) -> FlightView:
        ...
        dep_detail = details.get("airport", {}).get("origin", {}) if isinstance(details, dict) else {}
        arr_detail = details.get("airport", {}).get("destination", {}) if isinstance(details, dict) else {}
        ...
        airline_field = " ".join([x for x in [airline_name, airline_icao, airline_iata] if x]) or None
